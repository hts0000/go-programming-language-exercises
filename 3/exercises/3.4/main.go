...
// 练习 3.4
// 参考1.7节Lissajous例子的函数，构造一个web服务器
// 用于计算函数曲面然后返回SVG数据给客户端。
// 服务器必须设置Content-Type头部：w.Header().Set("Content-Type", "image/svg+xml")
// 根据地址源码修改：https://github.com/basicExploration/Demos/blob/master/gop_homework/ch3/%E7%AC%AC%E4%B8%89%E7%AB%A0%E7%BB%83%E4%B9%A0%E9%A2%98/%E7%BB%83%E4%B9%A03.4/main.go
// 访问方式:127.0.0.1:9000/?height=600&width=600   127.0.0.1:9000/download?height=600&width=600
...
func main() {
	http.HandleFunc("/", func(writer http.ResponseWriter, request *http.Request) {
		request.ParseForm()
		height, _ := strconv.Atoi(request.FormValue("height"))
		width, _ := strconv.Atoi(request.FormValue("width"))
		fmt.Println("打印第一", width, height)
		fmt.Fprintln(writer, TT(height, width))
	...
	http.HandleFunc("/download", func(writer http.ResponseWriter, request *http.Request) {
		...
		writer.Write([]byte(TT(height, width)))
